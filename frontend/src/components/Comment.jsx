...
function Comment() {
    ...
    async function handleComment() {
        try{
                let res = await axios.post(`http://localhost:3000/api/v1/blogs/comment/${blogId}`,
                    {
                        comment
                    },
                    {
    ...
    async function handleCommentLike(commentId) {
        console.log(commentId)
         try{
                let res = await axios.patch(`http://localhost:3000/api/v1/blogs/like-comment/${commentId}`,
                    {},
                    {
                        headers : {
                            // "Content-Type" : "multipart/form-data",
  ...
  return (
    <div className='bg-white h-screen fixed top-0 p-5 right-0 w-[400px] border-l drop-shadow-xl overflow-y-scroll'>
        ...
        <div className='my-4'>
            ...
            />
            <button onClick= {handleComment} className='bg-green-400 px-7py-3'>Add</button>
        </div>
